...
- `CATALOG_GOOGLE_BOOKS_API_KEY` — optional server-side Google Books API key. It is never returned to the browser.
...
Metadata editing uses `GET/PATCH /api/profiles/:profile/books/:book/metadata`, `POST .../metadata/reset`, and bounded raw `PUT/DELETE .../cover` operations with expected revision and source hash. Cover search uses fixed `google-books` and `open-library` adapters; candidate previews are relayed through a same-origin bounded endpoint, and import persists the selected bytes rather than hot-linking them. Every mutation uses optimistic revision and immutable-source-hash checks. The source download remains the original file and reports `X-Kindle-Bridge-Presentation-Version` separately so browser-local conversion can apply the effective metadata/cover to a derivative.

Back up the complete `/data` volume with the documented cold-backup helper or another SQLite-consistent volume snapshot. That volume now includes both SQLite overlay records and replacement-cover assets. `/cache` is intentionally excluded and rebuilds from the read-only source mounts. SIGTERM/SIGINT handling is installed before the initial scan. Shutdown stops new HTTP work and closes event streams first, cooperatively aborts scans and parser workers within the shared `CATALOG_SHUTDOWN_TIMEOUT_MS` deadline, retires any still-pending filesystem work from database access, and closes SQLite last.
