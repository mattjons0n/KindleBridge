...
The Kindle-resident cache is never authoritative by itself. A hit is accepted only for a book enumerated in the current live Documents hierarchy with the exact unadjusted path, object format, size, and valid modification timestamp; a changed, ambiguous, corrupt, missing, or removed object falls back safely to a bounded live read. Rotation retains a previously verified slot until its replacement has been written and read back exactly, and deletion authority is limited to a freshly revalidated Kindle Bridge cache slot. The cache files do not replace, rename, or delete books and do not touch Calibre's separate `metadata.calibre` file. A bounded same-origin IndexedDB cache remains a fallback, stores digest evidence rather than raw paths/serials, and can be cleared with browser site data. Neither cache is sent to the backend or cloud. A first-ever connection and genuine cache misses can therefore remain slower than later connections.

Each completed inventory adds a **Kindle metadata cache diagnostics** entry to the browser debug log. It reports only bounded aggregate counts and fixed outcome names: valid, missing, and malformed modification-date counts; portable-cache mismatch categories; separate Kindle/browser hit counts; browser lookup/write results; and A/B slot load/write status. When a Kindle returns unusable modification dates, the same entry includes a fixed timestamp-shape/character histogram, distinct-versus-repeated counts, the app-owned self-test object's returned shape, and exact aggregate stability counts against the previous connection in the same page. The probe samples at most 2,000 candidate books and keeps at most eight page-local device/storage snapshots. It deliberately excludes filenames, paths, titles, authors, identifiers, raw or per-object modification timestamps, device identity, cache keys, handles, checksums, and raw errors; none of the probe data is sent to the backend or persisted. Disconnect and reconnect without reloading the page to obtain the comparison. Cache acceptance and transfer safety remain unchanged.

KFX and AZW8 books remain visible Kindle presence evidence, but this release does not download their entire payload for metadata: the bounded reader supports PalmDB/MOBI containers, while real KFX requires a separate parser for its small `.sdr/assets/metadata.kfx` sidecar. Those objects keep metadata-based absence unknown rather than risking a false green check. A targeted bounded KFX reader and a physical `GetPartialObject` capability probe are tracked in [`BACKLOG.md`](BACKLOG.md).
