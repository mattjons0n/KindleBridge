...
Each completed inventory adds a **Kindle metadata cache diagnostics** entry to the browser debug log. It reports only bounded aggregate counts and fixed outcome names: valid, missing, and malformed modification-date counts; portable-cache mismatch categories; separate Kindle/browser hit counts; browser lookup/write results; and A/B slot load/write status. It deliberately excludes filenames, paths, titles, authors, identifiers, per-object modification timestamps, device identity, cache keys, handles, checksums, and raw errors. The diagnostics remain browser-local unless the user explicitly copies the debug log.
