...
Reconnect inventory uses conservative Calibre-style acceleration without changing Kindle contents. Kindle `.sdr` sidecar trees are pruned, Kindle Bridge derivatives with a valid managed token are not downloaded merely to recover weaker embedded fields, and object information already read by the current self-test collision scan is reused only after an exact live handle-set relist. Successfully parsed unmanaged metadata is cached in same-origin IndexedDB using a pseudonymous device identity and a digest of the exact unadjusted path, storage, size, and modification timestamp. A cache hit is accepted only for an object enumerated in the current live hierarchy; a changed size/time, adjusted path, missing timestamp, different device, or removed object misses safely. Raw serials and paths are not persisted, nothing is sent to the backend or cloud, the cache is bounded/expiring, and clearing browser site data removes it. A first connection and genuine cache misses can therefore remain slower than later connections.
