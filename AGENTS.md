...
- Kindle metadata acceleration stays browser-local and non-authoritative by itself. Reuse cached parsed fields only after current live enumeration and an exact pseudonymous-device/storage/unadjusted-path/size/modification-time match; never send this cache to the backend or use it to resurrect an absent object.
