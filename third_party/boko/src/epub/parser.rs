...
impl OpfParser {
    ...
    fn parse_spine_itemref(&mut self, e: &BytesStart) -> io::Result<()> {
        if let Some(idref) = attr(e, b"idref")? {
            if self.spine_ids.len() >= super::MAX_SPINE_CHAPTERS {
                return Err(io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!(
                        "resource limit exceeded: EPUB spine chapter count exceeds the {} chapter limit",
                        super::MAX_SPINE_CHAPTERS
                    ),
                ));
            }
...
mod tests {
    ...
    #[test]
    fn parse_opf_rejects_spine_itemref_n_plus_one_before_retention() {
        let mut opf = String::from(
            r#"<package xmlns="http://www.idpf.org/2007/opf"><metadata/><manifest><item id="c" href="c.xhtml" media-type="application/xhtml+xml"/></manifest><spine>"#,
        );
        for _ in 0..=super::super::MAX_SPINE_CHAPTERS {
            opf.push_str(r#"<itemref idref="c"/>"#);
        }
        opf.push_str("</spine></package>");

        let error = parse_opf(&opf).err().expect("hostile spine must fail");
        assert!(
            error
                .to_string()
                .contains("EPUB spine chapter count exceeds the 4096 chapter limit")
        );
    }
