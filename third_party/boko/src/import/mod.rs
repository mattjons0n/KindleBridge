...
pub trait Importer: Send + Sync {
    ...
    fn load_chapter(&self, id: ChapterId) -> crate::Result<Chapter> {
        ...
        compile_chapter_html(&html_bytes, base_path.as_deref(), &mut |path| {
...
pub(crate) fn compile_chapter_html(
    ...
    if html_bytes.len() > crate::dom::MAX_DOCUMENT_XHTML_BYTES {
        return Err(crate::Error::ResourceLimit {
            context: format!(
                "document XHTML exceeds the {} byte limit",
    ...
    let dom = crate::dom::parse_dom_limited(&html_str)?;
    ...
    let mut chapter = crate::dom::compile_dom_limited(&dom, &sheet_refs)?;
    ...
    if chapter.node_count() > crate::dom::MAX_DOCUMENT_IR_NODES {
        return Err(crate::Error::ResourceLimit {
            context: format!(
                "document IR node count exceeds the {} node limit",
                crate::dom::MAX_DOCUMENT_IR_NODES
            ),
        });
    }
    if chapter.semantics.len() > crate::dom::MAX_DOCUMENT_IR_ATTRIBUTES {
        return Err(crate::Error::ResourceLimit {
            context: format!(
                "document IR attribute count exceeds the {} attribute limit",
                crate::dom::MAX_DOCUMENT_IR_ATTRIBUTES
            ),
        });
    }
    if chapter.styles.len() > crate::dom::MAX_DOCUMENT_IR_STYLES {
        return Err(crate::Error::ResourceLimit {
            context: format!(
                "document IR style count exceeds the {} style limit",
                crate::dom::MAX_DOCUMENT_IR_STYLES
            ),
        });
    }
    if chapter.retained_text_bytes() > crate::dom::MAX_DOCUMENT_RETAINED_TEXT_BYTES {
        return Err(crate::Error::ResourceLimit {
            context: format!(
                "document retained IR text exceeds the {} byte limit",
                crate::dom::MAX_DOCUMENT_RETAINED_TEXT_BYTES
            ),
        });
    }
...
fn resolve_semantic_paths(chapter: &mut Chapter, base_path: &str) -> crate::Result<()> {
    let semantic_budget =
        crate::dom::MAX_DOCUMENT_RETAINED_TEXT_BYTES.saturating_sub(chapter.text_buffer().len());
    chapter
        .semantics
        .resolve_paths_bounded(semantic_budget, |path| {
...
mod tests {
    ...
    #[test]
    fn hostile_wide_flat_xhtml_fails_without_retaining_node_n_plus_one() {
        let mut html = String::from(
            r#"<?xml version="1.0"?><html xmlns="http://www.w3.org/1999/xhtml"><body>"#,
        );
        for _ in 0..crate::dom::MAX_DOCUMENT_DOM_NODES {
            html.push_str("<i/>");
        }
        html.push_str("</body></html>");

        let error = compile_chapter_html(html.as_bytes(), Some("wide.xhtml"), &mut |_| None)
            .expect_err("wide XHTML must fail closed");
        assert!(
            error
                .to_string()
                .contains("document DOM node count exceeds the 100000 node limit")
        );
    }

    #[test]
    fn semantic_path_expansion_is_checked_incrementally() {
        let mut html = String::from("<html><body>");
        for _ in 0..5_000 {
            html.push_str(r##"<a href="#x">x</a>"##);
        }
        html.push_str("</body></html>");
        let base = format!("{}.xhtml", "p".repeat(2_042));

        let error = compile_chapter_html(html.as_bytes(), Some(&base), &mut |_| None)
            .expect_err("expanded semantic paths must fail closed");
        assert!(
            error
                .to_string()
                .contains("document retained IR text exceeds")
        );
    }
