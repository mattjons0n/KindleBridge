...
impl Chapter {
    ...
    /// Allocate an importer-controlled node only when it fits the supplied
    /// document budget. The check happens before `Vec::push`, so the rejected
    /// N+1 node is never retained.
    pub(crate) fn alloc_node_bounded(
        &mut self,
        node: Node,
        max_nodes: usize,
    ) -> crate::Result<NodeId> {
        if self.nodes.len() >= max_nodes {
            return Err(crate::Error::ResourceLimit {
                context: format!("document IR node count exceeds the {max_nodes} node limit"),
            });
        }
    ...
    /// Append text only if the chapter's combined text and semantic-string
    /// storage remains within `max_retained_bytes`.
    pub(crate) fn append_text_bounded(
        &mut self,
        text: &str,
        max_retained_bytes: usize,
    ) -> crate::Result<TextRange> {
        self.ensure_retained_text_capacity(text.len(), max_retained_bytes)?;
        Ok(self.append_text(text))
    }
    ...
    /// Normalize and append text with a conservative pre-allocation check.
    /// Normalization never produces more bytes than its UTF-8 input, so
    /// reserving the input length proves the append cannot cross the cap.
    pub(crate) fn append_text_normalized_bounded(
        &mut self,
        text: &str,
        max_retained_bytes: usize,
    ) -> crate::Result<TextRange> {
        self.ensure_retained_text_capacity(text.len(), max_retained_bytes)?;
        Ok(self.append_text_normalized(text))
    }
    ...
    /// Total text-like bytes retained by this normalized chapter, including
    /// semantic attribute values stored in the sparse map.
    pub(crate) fn retained_text_bytes(&self) -> usize {
        self.text.len().saturating_add(self.semantics.buffer_len())
    }

    /// Aggregate canonical-math nodes and owned strings retained in the
    /// chapter's side table.
    pub(crate) fn math_retained_stats(&self) -> (usize, usize) {
        self.math.values().fold((0usize, 0usize), |acc, math| {
            let (nodes, bytes) = math.retained_stats();
            (acc.0.saturating_add(nodes), acc.1.saturating_add(bytes))
        })
    }

    /// Check a semantic attribute before its hash-map/string-buffer insert.
    /// Callers pass zero bytes for numeric/boolean side-table entries.
    pub(crate) fn ensure_semantic_capacity(
        &self,
        additional_bytes: usize,
        max_attributes: usize,
        max_retained_bytes: usize,
    ) -> crate::Result<()> {
        if self.semantics.len() >= max_attributes {
            return Err(crate::Error::ResourceLimit {
                context: format!(
                    "document IR attribute count exceeds the {max_attributes} attribute limit"
                ),
            });
        }
        self.ensure_retained_text_capacity(additional_bytes, max_retained_bytes)
    }

    fn ensure_retained_text_capacity(
        &self,
        additional_bytes: usize,
        max_retained_bytes: usize,
    ) -> crate::Result<()> {
        let next = self
            ...
            .ok_or_else(|| retained_text_limit(max_retained_bytes))?;
        if next > max_retained_bytes {
            return Err(retained_text_limit(max_retained_bytes));
        }
        Ok(())
    }
...
fn retained_text_limit(max_retained_bytes: usize) -> crate::Error {
    crate::Error::ResourceLimit {
        context: format!("document retained IR text exceeds the {max_retained_bytes} byte limit"),
    }
}
...
mod tests {
    ...
    #[test]
    fn bounded_node_and_text_appends_refuse_n_plus_one_before_growth() {
        let mut chapter = Chapter::new();
        let error = chapter
            .alloc_node_bounded(Node::new(Role::Paragraph), 1)
            .expect_err("root already consumes the one-node budget");
        assert!(error.to_string().contains("IR node count"));
        assert_eq!(chapter.node_count(), 1);

        chapter.append_text_bounded("abc", 3).unwrap();
        let error = chapter
            .append_text_bounded("d", 3)
            .expect_err("text N+1 must fail");
        assert!(error.to_string().contains("retained IR text"));
        assert_eq!(chapter.text_buffer(), "abc");
    }
}
