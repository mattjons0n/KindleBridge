...
impl SemanticMap {
    ...
    /// Bytes retained in the shared string-value buffer.
    ...
    where
        F: Fn(&str) -> String,
    {
        // The public low-level API remains infallible, but still resolves one
        // value at a time rather than retaining a second Vec<String> copy.
        let _ = self.resolve_paths_bounded(usize::MAX, resolver);
    }

    /// Bounded importer path resolution. Only compact `(NodeId, TextRange)`
    /// pairs are staged; each resolved String is checked and appended before
    /// the next one is allocated.
    pub(crate) fn resolve_paths_bounded<F>(
        &mut self,
        ...
        F: Fn(&str) -> String,
    {
        if self.buffer.len() > max_buffer_bytes {
            return Err(semantic_text_limit(max_buffer_bytes));
        }

        let src_entries: Vec<_> = self
            .src
            .iter()
            .map(|(&node, &range)| (node, range))
            .collect();
        for (node, old_range) in src_entries {
            let new_value = resolver(self.get_str(old_range));
            let range = self.append_bounded(&new_value, max_buffer_bytes)?;
            self.src.insert(node, range);
        }

        let href_entries: Vec<_> = self
            .href
            .iter()
            .map(|(&node, &range)| (node, range))
            .collect();
        for (node, old_range) in href_entries {
            let old_value = self.get_str(old_range);
            if old_value.contains("://") || old_value.starts_with("mailto:") {
                continue;
            }
            let new_value = resolver(old_value);
            let range = self.append_bounded(&new_value, max_buffer_bytes)?;
            self.href.insert(node, range);
        }
        Ok(())
    ...
    fn append_bounded(&mut self, value: &str, max_buffer_bytes: usize) -> crate::Result<TextRange> {
        let next = self
            .buffer
            .len()
            .checked_add(value.len())
            .ok_or_else(|| semantic_text_limit(max_buffer_bytes))?;
...
fn semantic_text_limit(max_buffer_bytes: usize) -> crate::Error {
    crate::Error::ResourceLimit {
        context: format!(
            "document retained IR text exceeds the {max_buffer_bytes} byte semantic-buffer limit"
        ),
    }
}
