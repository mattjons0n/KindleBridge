...
/// Maximum MathML source elements admitted in one normalized document. This
/// also bounds the temporary traversal stack before any `MathExpr` is built.
pub(crate) const MAX_DOCUMENT_MATH_SOURCE_ELEMENTS: usize = 4_096;
/// Maximum retained canonical expression nodes per document. The source cap
/// above proves the current fixed-arity builder cannot reach this ceiling
/// (each source element creates at most four expressions plus one root row).
pub(crate) const MAX_DOCUMENT_MATH_EXPR_NODES: usize = 20_000;
/// Maximum strings retained by all canonical math trees in one document.
pub(crate) const MAX_DOCUMENT_MATH_TEXT_BYTES: usize = 8 * 1024 * 1024;
/// Maximum MathML nesting accepted before any recursive conversion begins.
pub(crate) const MAX_DOCUMENT_MATH_DEPTH: usize = 128;
...
impl Math {
    ...
    /// Count canonical expression nodes and owned string bytes without
    /// recursion. Used by both per-document defense checks and book-cache
    /// aggregate accounting.
    pub(crate) fn retained_stats(&self) -> (usize, usize) {
        let mut nodes = 0usize;
        let mut bytes = self.alttext.as_ref().map_or(0, String::len);
        let mut stack = vec![&self.expr];
        while let Some(expr) = stack.pop() {
            nodes = nodes.saturating_add(1);
            match expr {
                MathExpr::Row(items) => stack.extend(items),
                MathExpr::Token { text, .. } => bytes = bytes.saturating_add(text.len()),
                MathExpr::Sub(a, b)
                | MathExpr::Sup(a, b)
                | MathExpr::Under { base: a, under: b }
                | MathExpr::Over { base: a, over: b }
                | MathExpr::Frac(a, b)
                | MathExpr::Root(a, b) => {
                    stack.push(a);
                    stack.push(b);
                }
                MathExpr::SubSup(a, b, c)
                | MathExpr::UnderOver {
                    base: a,
                    under: b,
                    over: c,
                ...
                MathExpr::Sqrt(expr) => stack.push(expr),
                MathExpr::Fenced { open, close, body } => {
                    bytes = bytes.saturating_add(open.len()).saturating_add(close.len());
                    stack.push(body);
                }
                MathExpr::Table { rows, .. } => {
                    for row in rows {
                ...
                MathExpr::Raw { mathml, latex } => {
                    bytes = bytes
                        .saturating_add(mathml.as_ref().map_or(0, String::len))
                        .saturating_add(latex.as_ref().map_or(0, String::len));
                }
                MathExpr::Space => {}
            }
        }
