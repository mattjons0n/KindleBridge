...
/// String-backed output that records a limit failure instead of performing
/// the N+1 allocation. Returning `fmt::Result` as `Ok` lets existing `write!`
/// call sites finish unwinding normally; callers inspect `failed` afterward.
...
impl SynthesisOutput {
    ...
    fn into_string(self) -> Option<String> {
        (!self.failed).then_some(self.inner)
    }
}

impl fmt::Write for SynthesisOutput {
    fn write_str(&mut self, value: &str) -> fmt::Result {
        ...
        if self.max_bytes.is_some_and(|max| next > max) {
            self.failed = true;
            return Ok(());
        }
...
fn synthesize_html_with_resolver<R: StyleResolver>(
    ...
    SynthesisResult {
        body: ctx
            .out
            .into_string()
            .expect("unbounded synthesis cannot exhaust an output budget"),
        assets: ctx.assets,
    }
}
...
/// Import/export pipeline variant which emits the complete document into one
/// capped buffer. It avoids the historical body→document copy and refuses the
/// N+1 write before the backing `String` can grow beyond `max_bytes`.
pub(crate) fn synthesize_xhtml_document_with_class_list_math_limited(
    ir: &Chapter,
    class_list: &[Option<&str>],
    title: &str,
    stylesheet_href: Option<&str>,
    ...
    let resolver = ClassListResolver { list: class_list };
    let mut ctx = SynthesisContext {
        out: SynthesisOutput::new(Some(max_bytes)),
        assets: HashSet::new(),
        ir,
        resolver: &resolver,
        indent_level: 0,
...
<head>
  ...
  <title>"#,
    ...
    escape_xml_into(&mut ctx.out, title);
    ctx.out.push_str("</title>\n");
    if let Some(href) = stylesheet_href {
        ctx.out
            .push_str("  <link rel=\"stylesheet\" type=\"text/css\" href=\"");
        escape_xml_into(&mut ctx.out, href);
        ctx.out.push_str("\"/>\n");
    }
    ctx.out.push_str("</head>\n<body>\n");
    for child_id in ir.children(NodeId::ROOT) {
        walk_node(child_id, &mut ctx, 0);
    }
    ...
    let body = ctx
        .out
        .into_string()
        .ok_or_else(|| crate::Error::ResourceLimit {
            context: format!(
                "normalized content: synthesized document exceeds the {max_bytes} byte limit"
            ),
        })?;
    Ok(SynthesisResult {
        body,
...
fn walk_node<R: StyleResolver>(id: NodeId, ctx: &mut SynthesisContext<'_, R>, depth: usize) {
    if ctx.out.failed || depth > crate::util::MAX_TREE_DEPTH {
        return;
    }
    ...
    if role == Role::Math {
        if let Some(math) = ctx.ir.math.get(&id) {
            if let Some(anchor) = ctx.ir.semantics.id(id) {
                ...
                escape_xml_into(&mut ctx.out, anchor);
                ctx.out.push_str("\">");
                match ctx.math_form {
                    MathForm::MathMl => {
                        let _ = crate::math::mathml::write_mathml_to(math, &mut ctx.out);
                    }
                    MathForm::Text => escape_xml_into(&mut ctx.out, &math.to_text()),
                }
...
pub fn escape_xml_into<W: fmt::Write>(out: &mut W, s: &str) {
    let mut rest = s;
    while let Some(i) = rest.find(['&', '<', '>', '"', '\'']) {
        let _ = out.write_str(&rest[..i]);
        let _ = out.write_str(match rest.as_bytes()[i] {
            b'&' => "&amp;",
            b'<' => "&lt;",
            b'>' => "&gt;",
            b'"' => "&quot;",
...
mod tests {
    ...
    #[test]
    fn capped_synthesis_output_refuses_n_plus_one_before_string_growth() {
        let mut out = SynthesisOutput::new(Some(3));
        out.push_str("abc");
        assert_eq!(out.inner.len(), 3);
        out.push('d');
        assert!(out.failed);
        assert_eq!(out.inner.len(), 3, "the rejected byte was never retained");
    }
