...
struct TransformContext<'a> {
    ...
    use_bloom: bool,
    /// Importer-controlled documents use fallible, pre-allocation IR guards;
    /// the public low-level compiler retains its historical infallible API.
    enforce_limits: bool,
...
impl<'a> TransformContext<'a> {
    ...
    fn transform(mut self) -> crate::Result<Chapter> {
    ...
    fn alloc_node(&mut self, node: Node) -> crate::Result<NodeId> {
        if self.enforce_limits {
            self.chapter
                .alloc_node_bounded(node, super::MAX_DOCUMENT_IR_NODES)
        } else {
            Ok(self.chapter.alloc_node(node))
        }
    }

    fn intern_style(&mut self, style: &ComputedStyle) -> crate::Result<crate::style::StyleId> {
        if self.enforce_limits {
            self.chapter
                .styles
                .intern_ref_bounded(style, super::MAX_DOCUMENT_IR_STYLES)
        } else {
            Ok(self.chapter.styles.intern_ref(style))
        }
    }
    ...
    fn append_text(
        ...
        text: &str,
        normalize: bool,
    ) -> crate::Result<crate::model::TextRange> {
        if self.enforce_limits {
            if normalize {
                self.chapter
                    .append_text_normalized_bounded(text, super::MAX_DOCUMENT_RETAINED_TEXT_BYTES)
            } else {
                self.chapter
                    .append_text_bounded(text, super::MAX_DOCUMENT_RETAINED_TEXT_BYTES)
            }
        } else if normalize {
            Ok(self.chapter.append_text_normalized(text))
        } else {
            Ok(self.chapter.append_text(text))
        }
    }

    /// Reserve one string-valued semantic entry before the setter appends to
    /// its buffer/hash map. Empty values are ignored by all semantic setters.
    fn reserve_semantic_string(&self, value: &str) -> crate::Result<bool> {
        if value.is_empty() {
            return Ok(false);
        }
        if self.enforce_limits {
            self.chapter.ensure_semantic_capacity(
                value.len(),
                super::MAX_DOCUMENT_IR_ATTRIBUTES,
                super::MAX_DOCUMENT_RETAINED_TEXT_BYTES,
            )?;
        }
        Ok(true)
    }

    /// Reserve one numeric/boolean semantic side-table entry.
    fn reserve_semantic_scalar(&self) -> crate::Result<()> {
        if self.enforce_limits {
            self.chapter.ensure_semantic_capacity(
                0,
                super::MAX_DOCUMENT_IR_ATTRIBUTES,
    ...
    fn process_node(
        ...
        if depth > super::MAX_DOCUMENT_TRANSFORM_DEPTH {
            return Ok(());
        }
        ...
        match &node.data {
            ArenaNodeData::Text(text) => {
                ...
                if text.trim().is_empty() && !preserve_whitespace {
                    ...
                    let range = self.append_text(" ", false)?;
                    let text_node = Node::text(range);
                    let ir_id = self.alloc_node(text_node)?;
            ...
            ArenaNodeData::Element { name, attrs, .. } => {
                    ...
                    || name.local.as_ref() == "math"
                {
                    ...
                    ir_node.style = self.intern_style(&computed)?;
                    let ir_id = self.alloc_node(ir_node)?;
                    self.chapter.append_child(ir_parent, ir_id);
                    // Preserve the element id for anchor/link resolution.
                    for attr in attrs {
                        if attr.name.local.as_ref() == "id" {
                            if self.reserve_semantic_string(&attr.value)? {
                                self.chapter.semantics.set_id(ir_id, &attr.value);
                            }
                        }
                    }
                    let mut expr = if self.enforce_limits {
                        let remaining_nodes = crate::math::MAX_DOCUMENT_MATH_EXPR_NODES
                            .saturating_sub(self.math_expr_nodes);
                        let remaining_bytes = crate::math::MAX_DOCUMENT_MATH_TEXT_BYTES
                            .saturating_sub(self.math_text_bytes);
                        crate::math::mathml::from_mathml_limited(
                            self.dom,
                            dom_id,
                            remaining_nodes,
                            remaining_bytes,
                        ...
                        crate::math::mathml::from_mathml(self.dom, dom_id)
                    };
                    if self.enforce_limits {
                        let (nodes, bytes) = expr.retained_stats();
                        self.math_expr_nodes += nodes;
                        self.math_text_bytes += bytes;
                    }
                ...
                for attr in attrs {
                    ...
                    match attr_name {
                        ...
                        "alt" => {
                            if self.reserve_semantic_string(&attr.value)? {
                                self.chapter.semantics.set_alt(ir_id, &attr.value);
                            }
                        }
                        "id" => {
                            if self.reserve_semantic_string(&attr.value)? {
                        ...
                        "title" => {
                            if self.reserve_semantic_string(&attr.value)? {
                                self.chapter.semantics.set_title(ir_id, &attr.value);
                            }
                        }
                        // Language (both lang and xml:lang)
                        "lang" => {
                            if self.reserve_semantic_string(&attr.value)? {
                                self.chapter.semantics.set_lang(ir_id, &attr.value);
                            }
                        }
                        ...
                        "class" if matches!(name.local.as_ref(), "code" | "pre") => {
                            for class in attr.value.split_whitespace() {
                                if let Some(lang) = class.strip_prefix("language-") {
                                    if self.reserve_semantic_string(lang)? {
...
pub fn transform(dom: &ArenaDom, stylesheets: &[(&Stylesheet, Origin)]) -> Chapter {
    let ctx = TransformContext::new(dom, stylesheets, false);
    ctx.transform()
        .expect("unbounded DOM transform cannot produce a resource-limit error")
}

/// Transform importer-controlled DOM while enforcing every IR retention
/// budget before the corresponding append/intern operation.
pub(crate) fn transform_limited(
    dom: &ArenaDom,
    stylesheets: &[(&Stylesheet, Origin)],
) -> crate::Result<Chapter> {
    TransformContext::new(dom, stylesheets, true).transform()
}
