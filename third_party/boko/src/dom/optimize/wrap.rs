...
use super::pass::try_walk_bottom_up;
...
pub fn wrap_mixed_content(chapter: &mut Chapter) {
    wrap_mixed_content_inner(chapter, false)
        .expect("unbounded mixed-content wrapping cannot exhaust an IR budget");
}

pub(crate) fn wrap_mixed_content_limited(chapter: &mut Chapter) -> crate::Result<()> {
    wrap_mixed_content_inner(chapter, true)
}

fn wrap_mixed_content_inner(chapter: &mut Chapter, limited: bool) -> crate::Result<()> {
    try_walk_bottom_up(chapter, |chapter, parent_id| {
        wrap_mixed_children(chapter, parent_id, limited)
    })
}

fn wrap_mixed_children(
    chapter: &mut Chapter,
    parent_id: NodeId,
    limited: bool,
) -> crate::Result<()> {
    ...
    if !has_inline || !has_block {
        return Ok(()); // No mixed content
    }

    // Find runs of consecutive inline children and wrap them
    wrap_inline_runs(chapter, parent_id, limited)
}
...
fn wrap_inline_runs(chapter: &mut Chapter, parent_id: NodeId, limited: bool) -> crate::Result<()> {
...
fn wrap_run(
    ...
    let inherited_style = chapter
        .node(parent_id)
        .map(|n| n.style)
        .and_then(|sid| chapter.styles.get(sid).cloned())
        .map(|parent| crate::style::inherit_from_parent(&parent));
    let wrapper_style = match inherited_style {
        Some(style) if limited => chapter
            .styles
            .intern_bounded(style, crate::dom::MAX_DOCUMENT_IR_STYLES)?,
        Some(style) => chapter.styles.intern(style),
        None => Default::default(),
    };
    let mut wrapper_node = Node::new(Role::Container);
    wrapper_node.style = wrapper_style;
    let wrapper_id = if limited {
        chapter.alloc_node_bounded(wrapper_node, crate::dom::MAX_DOCUMENT_IR_NODES)?
    } else {
        chapter.alloc_node(wrapper_node)
    };
