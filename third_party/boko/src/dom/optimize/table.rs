...
pub fn normalize_table_structure(chapter: &mut Chapter) {
    normalize_table_structure_inner(chapter, false)
        .expect("unbounded table normalization cannot exhaust an IR budget");
}

pub(crate) fn normalize_table_structure_limited(chapter: &mut Chapter) -> crate::Result<()> {
    normalize_table_structure_inner(chapter, true)
}

fn normalize_table_structure_inner(chapter: &mut Chapter, limited: bool) -> crate::Result<()> {
    ...
    for table_id in tables {
        normalize_single_table(chapter, table_id, limited)?;
    }
    Ok(())
}

/// Normalize a single table's structure.
fn normalize_single_table(
    chapter: &mut Chapter,
    table_id: NodeId,
    limited: bool,
) -> crate::Result<()> {
    ...
    if !header_rows.is_empty() {
        let head_id = alloc_wrapper(chapter, Role::TableHead, limited)?;
    ...
    if !body_rows.is_empty() {
        let body_id = alloc_wrapper(chapter, Role::TableBody, limited)?;
...
fn alloc_wrapper(chapter: &mut Chapter, role: Role, limited: bool) -> crate::Result<NodeId> {
    let node = Node::new(role);
    if limited {
        chapter.alloc_node_bounded(node, crate::dom::MAX_DOCUMENT_IR_NODES)
    } else {
        Ok(chapter.alloc_node(node))
    }
}
