...
pub fn detect_dropcaps(chapter: &mut Chapter) {
    detect_dropcaps_inner(chapter, false)
        .expect("unbounded dropcap detection cannot exhaust an IR budget");
}

pub(crate) fn detect_dropcaps_limited(chapter: &mut Chapter) -> crate::Result<()> {
    detect_dropcaps_inner(chapter, true)
}

fn detect_dropcaps_inner(chapter: &mut Chapter, limited: bool) -> crate::Result<()> {
    try_walk_bottom_up(chapter, |chapter, id| detect_one(chapter, id, limited))
}

fn detect_one(chapter: &mut Chapter, para_id: NodeId, limited: bool) -> crate::Result<()> {
    ...
    if let Some(base) = chapter.styles.get(para_style_id).cloned() {
        ...
        let new_id = if limited {
            chapter
                .styles
                .intern_bounded(styled, crate::dom::MAX_DOCUMENT_IR_STYLES)?
        } else {
            chapter.styles.intern(styled)
        };
