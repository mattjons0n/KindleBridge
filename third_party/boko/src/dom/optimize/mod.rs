...
pub fn optimize(chapter: &mut Chapter) {
    ...
    trim::trim_block_edges(chapter);
    dropcap::detect_dropcaps(chapter);
    prune::prune_empty(chapter);
}

/// Importer variant whose node/style-synthesizing passes reserve their IR
/// budget before allocation. The remaining passes only relink or remove data.
pub(crate) fn optimize_limited(chapter: &mut Chapter) -> crate::Result<()> {
    vacuum::vacuum(chapter);
    merge::merge_adjacent_spans(chapter);
    fuse::fuse_lists(chapter);
    wrap::wrap_mixed_content_limited(chapter)?;
    table::normalize_table_structure_limited(chapter)?;
    trim::trim_block_edges(chapter);
    dropcap::detect_dropcaps_limited(chapter)?;
    prune::prune_empty(chapter);
    Ok(())
}
