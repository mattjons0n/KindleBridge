...
/// Maximum encoded or decoded XHTML retained for one spine document (8 MiB).
pub(crate) const MAX_DOCUMENT_XHTML_BYTES: usize = 8 * 1024 * 1024;
/// Maximum DOM nodes retained for one spine document, including the root.
pub(crate) const MAX_DOCUMENT_DOM_NODES: usize = 100_000;
/// Maximum HTML/XML attributes retained for one spine document.
pub(crate) const MAX_DOCUMENT_ATTRIBUTES: usize = 100_000;
/// Maximum aggregate attribute name/value bytes retained per document (4 MiB).
pub(crate) const MAX_DOCUMENT_ATTRIBUTE_BYTES: usize = 4 * 1024 * 1024;
/// Maximum aggregate character data retained in one document DOM/IR (8 MiB).
pub(crate) const MAX_DOCUMENT_RETAINED_TEXT_BYTES: usize = 8 * 1024 * 1024;
/// Maximum normalized IR nodes retained for one spine document.
pub(crate) const MAX_DOCUMENT_IR_NODES: usize = 100_000;
/// Maximum semantic IR attributes retained for one spine document.
pub(crate) const MAX_DOCUMENT_IR_ATTRIBUTES: usize = 100_000;
/// Maximum distinct computed styles retained for one normalized document.
pub(crate) const MAX_DOCUMENT_IR_STYLES: usize = 25_000;
/// Maximum recursive DOM-to-IR transform depth. This is deliberately below
/// the general IR traversal ceiling because cascade frames are comparatively
/// large and importer input is untrusted.
pub(crate) const MAX_DOCUMENT_TRANSFORM_DEPTH: usize = 256;
...
/// Parse importer-controlled HTML/XHTML with allocation guards enabled.
///
/// The sink reserves every node, attribute and retained text append before it
/// grows the arena. If XHTML parsing yields no usable body, the independent
/// HTML fallback receives a fresh budget rather than a partially consumed one.
pub(crate) fn parse_dom_limited(html: &str) -> crate::Result<ArenaDom> {
    if html.len() > MAX_DOCUMENT_XHTML_BYTES {
        return Err(crate::Error::ResourceLimit {
            context: format!("document XHTML exceeds the {MAX_DOCUMENT_XHTML_BYTES} byte limit"),
        });
    }
...
/// Fallible importer variant of [`compile_dom`]. All transform and optimizer
/// retention is checked before the N+1 node/style/text/semantic allocation.
pub(crate) fn compile_dom_limited(
    dom: &ArenaDom,
    ...
    let mut chapter = transform::transform_limited(dom, &all_stylesheets)?;
    optimize::optimize_limited(&mut chapter)?;
    Ok(chapter)
}
