...
pub enum Error {
    ...
    /// An otherwise parseable input or generated output exceeded a bounded
    /// resource envelope. These failures are deterministic and fail closed.
    #[error("resource limit exceeded: {context}")]
    ResourceLimit {
...
impl From<Error> for std::io::Error {
    fn from(e: Error) -> Self {
        ...
        match e {
            ...
            Error::ResourceLimit { .. } => std::io::Error::new(std::io::ErrorKind::InvalidData, e),
        }
    }
}
...
mod tests {
    ...
    fn kind_mapping() {
        ...
        let err: std::io::Error = Error::ResourceLimit {
            context: "document too large".into(),
        }
        .into();
        assert_eq!(err.kind(), std::io::ErrorKind::InvalidData);
    }
