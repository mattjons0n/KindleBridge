...
- Repeat inventory prunes `.sdr` sidecar descendants, skips redundant reads of managed Kindle Bridge derivatives, reuses current-session collision metadata only after an exact live handle-set relist, and maintains a bounded browser-local metadata cache. Cache reuse requires the current live object plus the same pseudonymous device, storage, exact unadjusted path, size, and valid modification timestamp. Evidence keys are digests rather than raw paths/serials, the backend receives none of it, and clearing site data removes the cache. KFX/AZW8 remain visible but conservatively metadata-incomplete because the current PalmDB/MOBI parser cannot read their containers; downloading the entire unsupported book is avoided.
...
- Raw Kindle inventory and USB work remain in the browser. The backend never controls USB or receives raw device serials/book bytes for reconciliation. Only the bounded acceleration metadata described above persists locally in the browser; live hierarchy enumeration is still required on every connection.
...
- The authoritative 2026-08-30 root `npm run check` passes 50/50 test files and 563/563 tests, followed by successful client typechecking, server build, and production Vite build. Do not reuse the historical 18-file/133-test POC count.
...
- `client/src/kindle/inventory.ts`, `client/src/kindle/metadata-cache.ts`, and `client/src/kindle/matching.ts` — device enumeration, browser-local acceleration, and three-state evidence
