...
Deferred product and engineering decisions are tracked in [`BACKLOG.md`](BACKLOG.md). The current safe-write behavior remains authoritative until a backlog item is explicitly implemented and accepted.
...
- Repeat inventory prunes `.sdr` sidecar descendants, skips redundant reads of managed Kindle Bridge derivatives, and reuses current-session collision metadata only after an exact live handle-set relist. It also reads a bounded, checksum-protected A/B metadata cache from the selected Kindle storage root. Portable hits require a currently enumerated object with the exact unadjusted Documents-relative path, object format, size, and valid modification timestamp; corrupt, conflicting, stale, or absent entries never supply evidence. A bounded same-origin IndexedDB cache keyed by pseudonymous device/storage and digest evidence remains the fallback. The root cache contains clear relative paths and parsed fields but no serials, browser identity, handles, host paths, credentials, or book bytes; neither cache reaches the backend/cloud. KFX/AZW8 remain visible but conservatively metadata-incomplete because the current PalmDB/MOBI parser cannot read their containers; downloading the entire unsupported book is avoided.
- Every completed inventory writes one browser-local, privacy-safe cache diagnostic summary to the debug log. Aggregate evidence eligibility, portable mismatch categories, separate Kindle/browser hits, browser cache operations, and A/B load/write outcomes are included. A bounded page-local modification-date probe additionally reports fixed shape/character categories, distinct/constant counts, the app-owned self-test object's returned shape, and exact stability counts against the previous same-page connection. Book/device strings, paths, raw or per-object modification timestamps, identities, handles, cache keys, checksums, and raw errors are explicitly omitted; the probe is neither persisted nor sent to the backend. Cache matching and mutation authority are unchanged.
...
- The authoritative 2026-08-30 root `npm run check` passes 54/54 test files and 639/639 tests, followed by successful client typechecking, server build, and production Vite build. Do not reuse the historical 18-file/133-test POC count.
