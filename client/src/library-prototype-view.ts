...
function renderLibraryNav(snapshot: CatalogBrowserSnapshot): string {
  ...
  const items: ReadonlyArray<readonly [LibraryView, string, string, number | undefined]> = [
    ...
    ["series", "≋", "Series", snapshot.seriesPage?.total],
    ["attention", "!", "Needs attention", snapshot.healthPage?.counts.active || undefined],
...
function renderSmartShelfRail(snapshot: CatalogBrowserSnapshot): string {
  ...
  return `<div class="library-sidebar-label library-shelves-label">Shelves</div><nav class="library-shelf-list" aria-label="Smart shelves">${shelves.map((shelf) => {
    const active = snapshot.activeShelf?.id === shelf.id;
    const count = "serverCount" in shelf ? shelf.serverCount : null;
    return `<button type="button" class="library-shelf-item${active ? " active" : ""}" data-ui-action="apply-smart-shelf" data-shelf-id="${escapeHtml(shelf.id)}"${active ? ' aria-current="page"' : ""}><span aria-hidden="true">${shelf.id === "builtin-favorites" ? "★" : shelf.id === "builtin-want-to-read" ? "◇" : shelf.id === "builtin-missing-cover" ? "▧" : "⌁"}</span><span>${escapeHtml(shelf.name)}</span>${count === null ? "" : `<small>${count}</small>`}</button>`;
...
function renderCoverProviderSettings(snapshot: CatalogBrowserSnapshot): string {
  ...
  return `<details class="settings-provider-card" data-status="${tone}"${settings.editing ? " open" : ""}><summary><span><strong>Online cover search</strong><small>Google Books API key</small></span><span class="settings-provider-status"><i aria-hidden="true"></i>${escapeHtml(status)}</span></summary><div class="settings-provider-body">${locked ? '<p>This server has locked Settings. Provider credentials can only be viewed here.</p>' : settings.editing ? `<label><span>Google Books API key</span><input id="settings-google-books-key" type="password" value="" autocomplete="new-password" spellcheck="false" placeholder="Paste API key"${settings.busy ? " disabled" : ""} /><small>The saved key is never returned to this browser. Leaving this field closes it empty.</small></label>${settings.error ? `<p class="settings-provider-error" role="alert">${escapeHtml(settings.error)}</p>` : ""}<div class="settings-provider-actions"><button type="button" data-ui-action="cancel-google-books-key"${settings.busy ? " disabled" : ""}>Cancel</button>${configured ? `<button type="button" class="danger" data-ui-action="remove-google-books-key"${settings.busy ? " disabled" : ""}>Remove</button>` : ""}<button type="button" class="primary" data-ui-action="save-test-google-books-key"${settings.busy ? " disabled" : ""}>${settings.busy ? "Saving and testing…" : "Save & test"}</button></div>` : `<p>${configured ? `A server-stored key is configured${provider?.lastTestedAt ? ` and was last tested ${escapeHtml(relativeScanTime(provider.lastTestedAt))}` : ""}. Kindle Bridge only displays a fixed mask.` : "Open Library works without a key. Add a key only if you also want Google Books results."}</p>${settings.error ? `<p class="settings-provider-error" role="alert">${escapeHtml(settings.error)}</p>` : ""}<div class="settings-provider-actions">${configured ? `<button type="button" class="danger" data-ui-action="remove-google-books-key"${disable ? " disabled" : ""}>Remove</button>` : ""}<button type="button" data-ui-action="edit-google-books-key"${disable ? " disabled" : ""}>${configured ? "Replace key" : "Add key"}</button></div>`}</div></details>`;
}
...
export function renderOnboarding(snapshot: CatalogBrowserSnapshot, state: AppState): string {
  ...
  const content = wizard.step === "welcome"
    ? `<h2>Welcome to Kindle Bridge</h2><p>Set up your household library in a few steps. Your original books stay untouched, and no account or provider API key is required.</p>`
    : wizard.step === "library"
...
function renderMatchReview(snapshot: CatalogBrowserSnapshot, state: AppState): string {
  ...
  const deviceSection = item
    ...
    : '<section class="match-review-device"><span class="library-sheet-eyebrow">Device evidence</span><h3>No exact Kindle file identified</h3><p>The incomplete scan cannot safely prove that this catalog book is absent, so Kindle Bridge keeps it yellow instead of risking a duplicate.</p></section>';
...
function renderMetadataEditor(snapshot: CatalogBrowserSnapshot, state: AppState): string {
  ...
  const updateNote = editedPresentation
    ...
    : '<div class="metadata-existing-copy-note" role="note"><strong>Library originals are never rewritten.</strong><span>Save an EPUB metadata or cover edit first. A guarded Update Kindle copy action appears when exactly one prior Kindle Bridge-managed presentation is connected.</span></div>';
  const formatNotice = data.book.format.toLocaleLowerCase() === "azw3"
    ...
    : '<div class="metadata-format-note" role="note"><strong>Applied on Send</strong><span>For EPUB, Kindle Bridge embeds these edits in a temporary browser-created derivative. The source EPUB remains byte-for-byte unchanged.</span></div>';
  const fields = [
  ...
  return `<div class="library-modal-backdrop"${editor.busy ? "" : ' data-ui-action="close-metadata-editor"'} aria-hidden="true"></div><section class="library-metadata-sheet" role="dialog" aria-modal="true" aria-labelledby="metadata-editor-title" aria-describedby="metadata-editor-description" tabindex="-1">
    ...
    <header class="metadata-editor-head"><div><div class="library-sheet-eyebrow">Presentation overlay</div><h2 id="metadata-editor-title">Edit metadata &amp; cover</h2><p id="metadata-editor-description">Change how “${escapeHtml(data.book.title)}” appears in Kindle Bridge and future browser-created transfers.</p></div><span class="metadata-revision-chip">Revision ${data.revision}</span></header>
    ...
    <div class="metadata-editor-body"><section class="metadata-fields-panel" aria-labelledby="metadata-fields-title"><div class="metadata-section-head"><div><h3 id="metadata-fields-title">Book details</h3><p>Turn on an override only for fields Kindle Bridge should replace.</p></div><span>${Object.keys(draft).length} overridden</span></div><form class="metadata-editor-form" data-metadata-book-id="${escapeHtml(editor.bookId)}" novalidate>${fields}</form><div class="metadata-panel-actions"><button type="button" data-ui-action="reset-book-metadata"${mutationDisabled || Object.keys(data.overrides).length === 0 ? " disabled" : ""}>Reset metadata</button><button type="button" class="primary" data-ui-action="save-book-metadata"${mutationDisabled ? " disabled" : ""}>${editor.busy ? "Saving…" : "Save metadata"}</button></div>${renderMetadataCandidateDiscovery(snapshot)}</section>
...
function renderBookDetails(snapshot: CatalogBrowserSnapshot, state: AppState): string {
  ...
  return `<div class="library-modal-backdrop" data-ui-action="close-book-details" aria-hidden="true"></div><aside class="library-book-details-sheet" role="dialog" aria-modal="true" aria-labelledby="book-details-title" aria-describedby="book-details-subtitle" tabindex="-1">
    ...
    <section class="book-details-section" aria-labelledby="book-details-kindle"><h3 id="book-details-kindle">Kindle comparison</h3><p class="book-details-kindle-status" data-status="${escapeHtml(actions.kindleStatus)}">${escapeHtml(kindleLabel)}</p>${kindleItems.length ? `<ul class="book-details-kindle-files">${kindleItems.map((item) => `<li><span><strong>${escapeHtml(item.title ?? item.filename)}</strong><code>${escapeHtml(item.filename)}</code></span><small>${escapeHtml(formatCatalogBytes(item.size))} · ${item.managed ? "Kindle Bridge transfer" : "Existing device file"}</small>${actions.kindleStatus === "possible" ? `<button type="button" data-ui-action="open-match-review" data-item-id="${escapeHtml(item.id)}" data-book-id="${escapeHtml(book.id)}"${actions.matchReview.enabled ? "" : ` disabled title="${escapeHtml(actions.matchReview.reason ?? "Unavailable")}"`}>Review match</button>` : ""}</li>`).join("")}</ul>` : `<p class="book-details-provenance">${snapshot.kindleInventory ? `No associated object in the inventory scanned ${escapeHtml(relativeScanTime(snapshot.kindleInventory.scannedAt))}.` : "Connect a Kindle to compare this title with its Documents."}</p>${actions.kindleStatus === "possible" ? `<button type="button" data-ui-action="open-match-review" data-item-id="${escapeHtml(catalogPossibleMatchReviewId(book.id))}" data-book-id="${escapeHtml(book.id)}"${actions.matchReview.enabled ? "" : ` disabled title="${escapeHtml(actions.matchReview.reason ?? "Unavailable")}"`}>Why is this a possible match?</button>` : ""}`}${latestDelivery ? `<div class="book-details-last-delivery"><strong>Last verified transfer</strong><span>${escapeHtml(latestDelivery.filename ?? "Recorded Kindle Bridge transfer")} · ${latestDelivery.size === undefined ? "size not recorded" : escapeHtml(formatCatalogBytes(latestDelivery.size))} · ${escapeHtml(relativeScanTime(latestDelivery.deliveredAt))}</span><small>${latestDelivery.currentPresentation ? "Matches the current catalog presentation" : "A prior catalog presentation; use Update Kindle copy after connecting"}</small></div>` : ""}</section>
    ...
    const matchLabel = staleMatch
      ...
      : item.match === "confirmed"
        ...
        : item.stalePresentation
          ? "Prior Kindle Bridge presentation"
        : item.match === "possible"
          ? "Possible match"
          : item.managed
            ? "Managed item"
...
function renderMetadataLookupJobs(snapshot: CatalogBrowserSnapshot): string {
  ...
  }).join("")}</ol><p class="metadata-job-safety">Suggestions remain review-only. Kindle Bridge never changes metadata or covers until you choose fields and import them.</p></section>`;
...
export function renderLibraryPrototype(
  ...
  const compatibilityNotice = webUsbUsable
    ...
    : `<div class="notice warning library-compatibility-notice" role="status"><div><strong>Kindle connection unavailable</strong>${state.secureContext
      ...
      : "Open Kindle Bridge over trusted HTTPS or localhost."} Browsing, metadata editing, and Send later remain available.</div></div>`;
