...
import type { CatalogHealthIssue, MetadataLookupJob } from "./catalog-client";
...
function renderHardcoverSeriesChoice(snapshot: CatalogBrowserSnapshot, job: MetadataLookupJob, entry: MetadataLookupJob["entries"][number]): string {
  if (job.provider !== "hardcover" || entry.acceptedAt || !entry.candidates.length) return "";
  const book = snapshot.healthBooks.get(entry.bookId);
  const review = snapshot.hardcoverBulkReview?.jobId === job.id ? snapshot.hardcoverBulkReview : undefined;
  const choice = review?.selections.get(entry.bookId);
  const disabled = review?.busy || snapshot.metadataLookupBusy || !book || snapshot.serviceStatus?.settingsMode === "read-only";
  const error = review?.errors.get(entry.bookId);
  return `<div class="hardcover-series-choice"><span class="hardcover-current-series">Current: ${escapeHtml(book?.series ? `${book.series}${book.seriesIndex == null ? "" : ` · Volume ${book.seriesIndex}`}` : "No series")}</span><label><span>Series to use</span><select data-ui-action="select-hardcover-bulk-series" data-book-id="${escapeHtml(entry.bookId)}"${disabled ? " disabled" : ""}><option value="">Choose a book and series…</option>${entry.candidates.filter((candidate) => candidate.metadata.series && Object.hasOwn(candidate.metadata, "seriesIndex")).map((candidate) => `<option value="${escapeHtml(candidate.candidateId)}"${choice?.candidateId === candidate.candidateId ? " selected" : ""}>${escapeHtml(candidate.metadata.series)} · ${candidate.metadata.seriesIndex == null ? "Volume not listed" : `Volume ${candidate.metadata.seriesIndex}`} — ${escapeHtml(candidate.metadata.title ?? "Untitled")} · ${escapeHtml(candidate.metadata.authors?.join(", ") ?? "Unknown author")} (${escapeHtml(candidate.confidence)} confidence)</option>`).join("")}</select></label><label class="hardcover-replace-choice"><input type="checkbox" data-ui-action="replace-hardcover-bulk-series" data-book-id="${escapeHtml(entry.bookId)}"${choice?.replaceExisting ? " checked" : ""}${disabled || !choice ? " disabled" : ""} /><span>Replace existing series <small>Otherwise, fill missing fields only.</small></span></label>${error ? `<p class="health-error" role="alert">${escapeHtml(error)}</p>` : ""}</div>`;
}

function renderHardcoverBulkApply(snapshot: CatalogBrowserSnapshot, job: MetadataLookupJob): string {
  if (job.provider !== "hardcover" || !job.entries.some((entry) => entry.candidates.length)) return "";
  const review = snapshot.hardcoverBulkReview?.jobId === job.id ? snapshot.hardcoverBulkReview : undefined;
  const count = review?.selections.size ?? 0;
  const disabled = !count || review?.busy || snapshot.metadataLookupBusy || snapshot.serviceStatus?.settingsMode === "read-only";
  return `<div class="hardcover-bulk-apply"><div><strong>${review?.busy ? `Applying ${review.completed ?? 0} of ${review.total ?? count}` : `${count} ${count === 1 ? "book" : "books"} selected`}</strong><p>Choose one series per book above. Existing choices are kept unless you opt to replace them. Only series and volume are applied; originals stay unchanged.</p>${review?.summary ? `<p role="status">${escapeHtml(review.summary)}</p>` : ""}</div><button type="button" class="primary" data-ui-action="apply-hardcover-bulk-series"${disabled ? " disabled" : ""}>${review?.busy ? "Applying…" : "Apply selected series"}</button></div>`;
}
...
function renderLookupJobs(snapshot: CatalogBrowserSnapshot): string {
  ...
  const detail = !active ? "" : `<section class="metadata-job-detail" aria-labelledby="metadata-job-detail-title"><header><div><h3 id="metadata-job-detail-title">Book suggestions</h3><p>${active.ready} ready to review · ${active.pending} waiting${active.failed ? ` · ${active.failed} couldn't be searched` : ""}${active.noResults ? ` · ${active.noResults} with no results` : ""}</p></div><button type="button" data-ui-action="close-metadata-job" aria-label="Close search details">×</button></header>
    ...
    <ol class="metadata-job-entries">${active.entries.map((entry) => {
      ...
      return `<li data-status="${escapeHtml(entry.status)}"><span><strong>${escapeHtml(book?.title ?? `Book ${entry.rank + 1}`)}</strong><small>${entry.acceptedAt ? "Changes applied" : escapeHtml(lookupStatus(entry.status))}</small>${entry.errorCode ? `<details class="health-search-error"><summary>Why did this fail?</summary><span>${escapeHtml(entry.errorCode === "provider-unauthorized" ? "The Hardcover token is invalid or expired. Replace it in Settings." : entry.errorCode === "provider-forbidden" ? "The Hardcover token needs book and series read permissions. Check its scopes in Hardcover account settings." : entry.errorCode === "provider-rate-limited" ? "Hardcover is limiting requests. Completed suggestions are saved; retry failed searches later." : entry.errorCode)}</span></details>` : ""}</span>${renderHardcoverSeriesChoice(snapshot, active, entry)}${candidate && !entry.acceptedAt ? `<button type="button" data-ui-action="review-metadata-job-candidate" data-job-id="${escapeHtml(active.id)}" data-book-id="${escapeHtml(entry.bookId)}" data-candidate-id="${escapeHtml(candidate.candidateId)}"${snapshot.hardcoverBulkReview?.busy ? " disabled" : ""}>Review ${entry.candidates.length} ${entry.candidates.length === 1 ? "suggestion" : "suggestions"}</button>` : ""}</li>`;
    }).join("")}</ol>${renderHardcoverBulkApply(snapshot, active)}<p class="metadata-job-safety">You choose which details and covers to use. Nothing is changed until you review and apply a suggestion.</p></section>`;
  return `<details class="health-searches"${open ? " open" : ""}><summary>Online book searches<span>${active ? escapeHtml(lookupStatus(active.status)) : `${summaries.length} recent`}</span></summary><div class="health-searches-body"><div class="attention-section-head"><div><h2>Search history</h2><p>Find covers and book details, then review the suggestions here.</p></div><button type="button" data-ui-action="reload-metadata-jobs"${snapshot.metadataLookupState === "loading" ? " disabled" : ""}>Refresh</button></div>${snapshot.metadataLookupError && !active ? `<div class="health-error" role="alert">${escapeHtml(snapshot.metadataLookupError)}</div>` : ""}${snapshot.metadataLookupState === "loading" && !page ? '<p role="status">Loading searches…</p>' : summaries.length === 0 ? '<p>No searches yet. Use a book’s options to search for a cover or book details.</p>' : `<div class="metadata-job-list">${summaries.map((job) => `<button type="button" data-ui-action="open-metadata-job" data-job-id="${escapeHtml(job.id)}"${active?.id === job.id ? ' aria-current="true"' : ""}><span><strong>${escapeHtml(job.provider === "hardcover" ? "Hardcover" : job.provider === "google-books" ? "Google Books" : "Open Library")}</strong><small>${escapeHtml(relativeTime(job.updatedAt))}</small></span><span>${job.ready} of ${job.total} ready</span><em data-status="${escapeHtml(job.status)}">${escapeHtml(lookupStatus(job.status))}</em></button>`).join("")}</div>`}${detail}</div></details>`;
}
